...
%
% Expert rules for the chisq task 
%
...
feedback(ppool, [_S_VR, _S_Box, _N_VR, _N_Box], _Col, F)
 => F = "Correctly identified the pooled proportion of successes.".
...
% Fourth step: determine the chi2ratio
expert(chisq, stage(3), From, To, [step(expert, chi2ratio, [])]) :-
    From = chi2ratio0(Z),
    To = chi2ratio(Z^2).

feedback(chi2ratio, [], Col, F)
 => F = [ "Correctly identified ",
          "the ", \nowrap([\mmlm(Col, chi2), "-statistic."]) 
        ].

hint(chi2ratio, Col, H)
 => H = [ "The ", \nowrap([\mmlm(Col, chi), "-statistic"]), " is based on the ",
          \nowrap([\mmlm(Col, z), "-statistic."])
        ].

%
% Buggy-Rules for the for the chisq task
%
/* % Buggy-Rule: difference in numerator and denominator of p_pool instead of addition. Appeared 1-2x in 2018 exams.
Does not work yet, because results in 1.5Nan in the zstat step. Requires not yet implemented feature of conditional
inclusion of buggy rule.
...
hint(pdiff, Col, H)
  => H = [ "Do not subtract the numbers in the numerator and ",
           "denominator when calculating the pooled proportion of successes ", 
           \mmlm(Col, color(pdiff, p_pool))
         ]. */


% Buggy-Rule: forgot to square z. 
buggy(chisq, stage(3), From, To, [step(buggy, square, [Z])]) :-
    From = chi2ratio0(Z),
...
hint(square, Col, H) 
 => H = [ "Do not forget to raise the ",
           \nowrap([\mmlm(Col, color(square, z)), "-value"]),
           " to the square to obtain the ",
           \nowrap([\mmlm(Col, color(square, chi2)), "-statistic."])
        ].

% Buggy-Rule: forgot to square z (abs)
buggy(chisq, stage(3), From, To, [step(buggy, square2, [Z])]) :-
    From = chi2ratio0(Z),
...
% Buggy-Rule: added probabilities instead of subtracting them. 
...
% Buggy-Rule: forgot parentheses around (1/N_VR + 1/N_Box). 
buggy(chisq, stage(2), From, To, [step(buggy, paren2, [N_VR, N_Box])]) :-
    From = A * B * (frac(1, N_VR) + frac(1, N_Box)),
...
hint(paren2, Col, H)
 => H = [ "Do not forget to add parentheses around ", 
...
feedback(school1, [N_VR, N_Box], Col, F)
 => F = [ "The addition between ", \mmlm(Col, frac(1, N_VR)), " and ", \mmlm(Col, frac(1, N_Box)), 
          " was not correct. Please keep in mind that ", 
          \mmlm(Col, [color(school, color("black", frac(1, N_VR)) + color("black", frac(1, N_Box)))
            =\= frac(1, color(school, color("black", N_VR) + color("black", N_Box))), "."])
        ].

hint(school1, Col, H)
 => H= [ "Please do not forget school ",
...
buggy(chisq, stage(2), From, To, [step(buggy, paren3, [From])]) :-
    From = P_Pool * (1 - P_Pool) * (frac(1, N_VR) + frac(1, N_Box)),
...
hint(paren3, _Col, H)
 => H = [ "Remember the parentheses around the different ",
...
hint(flip, _Col, H)
 => H = [ "Do not flip the numerator and denominator of the test statistic." ].
