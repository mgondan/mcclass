...
render
--> { start(item(_T0, _S_T0, _EOT, _S_EOT, _D, _S_D, N, _Mu)) },
    html(
...
buggy(tratio, tage(2), X, Y, [step(buggy, mu, [Mu])]) :-
    X = paired(D, Mu, S_D, N),
    Y = tstat(dfrac(omit_right(mu, D - Mu), S_D / sqrt(N))).

feedback(mu, [Mu], Col, F)
 => F = [ "In the ", \mmlm(hyph(t, "ratio,")), " the null ",
...
feedback(bug1, [D, Mu, S, SQRT_N], Col, F)
 => F = [ "Please do not forget the parentheses around the numerator and ",
...
feedback(t0, [D, _T0], Col, F)
 => F = [ "Please insert the average change ",
...
feedback(s_t0, [S, _S_T0], Col, F)
 => F = [ "Please insert the standard deviation of the change ",
...
feedback(sqrt1, [N], Col, F)
 => F = [ "Please do not forget the square root around ", 
