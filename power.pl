	  ...
	  [ div(class(card), div(class('card-body'),
		...
		p(class('card-text'),
		  [ "“Laparoscopy-naïve medical students were randomized into ",
		    ...
		    \mmlm([task(power), digits(0)], N_BOX = r(n_box)), ") used E-learning for ", 
		    "laparoscopic cholecystectomy and practiced ",
		    "basic skills with Box trainers. The VR group (", 
		    \mmlm([task(power), digits(0)], N_VR = r(n_vr)), ") trained ",
		    ...
		    "(VR: ", \mmlm([task(power), digits(1)], r(vr)), " ± ", \mmlm([task(power), digits(1)], r(s_vr)), 
		    " vs. BOX: ", \mmlm([task(power), digits(1)], r(box)), " ± ", \mmlm([task(power), digits(1)], r(s_box)), 
